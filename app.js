...
async function load() {
  try {
    ...
    halls = await api("wedding_halls?select=hall_id,name,sido,sigungu,road_address,phone,website,venue_type&is_public=eq.true&operation_status=eq.%EC%9A%B4%EC%98%81&order=name.asc&limit=500");
...
function setupRegions() {
  ...
  if (rb) {
    ...
    rb.onclick = e => { if (e.target.dataset.region) { $("#sido").value=e.target.dataset.region; showList(); render(); location.hash="rankings"; } };
  }
}
...
async function showDetail(id){
 ...
 $("#detailName").textContent=h.name; $("#detailArea").textContent=[h.sido,h.sigungu].filter(Boolean).join(" "); $("#detailAddress").textContent=h.road_address||"주소 확인중"; $("#detailPhone").textContent=h.phone||"전화번호 확인중";
 $("#detailWebsite").innerHTML=h.website?`<a href="${esc(h.website)}" target="_blank" rel="noopener">공식/참고 사이트 열기 ↗</a>`:"홈페이지 확인중";
 try{
  const results=await Promise.allSettled([api(`hall_rooms?select=room_name,floor,capacity_min,capacity_max,ceremony_type,meal_type&hall_id=eq.${encodeURIComponent(id)}`),api(`wedding_prices?select=effective_date,rental_fee,decor_fee,meal_price_per_person,minimum_guarantee,source_name,source_url,notes&hall_id=eq.${encodeURIComponent(id)}&order=effective_date.desc`),api(`reviews?select=visit_role,food_score,parking_score,access_score,facility_score,bride_waiting_score,banquet_score,service_score,value_score,overall_score,review_text,created_at&hall_id=eq.${encodeURIComponent(id)}&order=created_at.desc&limit=100`)]);
  const rooms=results[0].status==="fulfilled"?results[0].value:[], prices=results[1].status==="fulfilled"?results[1].value:[], reviews=results[2].status==="fulfilled"?results[2].value:[];
  ...
  renderReviews(reviews); renderAuth();
...
function renderReviews(reviews){
 ...
 if(!reviews.length){box.innerHTML='<div class="pending big">아직 등록된 평가가 없습니다.<br><small>실제 이용자 평가가 쌓이면 랭킹에 반영됩니다.</small></div>';list.innerHTML="";return}
 const metrics=[["음식","food_score"],["교통/접근성","access_score"],["주차","parking_score"],["예식홀/시설","facility_score"],["신부대기실","bride_waiting_score"],["연회장","banquet_score"],["서비스","service_score"],["가성비","value_score"]],overall=avg(reviews,"overall_score");
...
parseAuthHash(); restoreSession(); load();

async function authApi(path,options={}){const key=getKey(),url=`${getUrl()}/auth/v1/${path}`;const h={apikey:key,"Content-Type":"application/json",...(options.headers||{})};if(accessToken)h.Authorization=`Bearer ${accessToken}`;const r=await fetch(url,{...options,headers:h});const t=await r.text();let data={};try{data=t?JSON.parse(t):{}}catch(_){}if(!r.ok)throw new Error(data.msg||data.message||`Auth ${r.status}`);return data}
