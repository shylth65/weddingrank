...
async function sendMagicLink(){
  ...
  try{const redirectUrl="https://shylth65.github.io/weddingrank/";await authApi("otp?redirect_to="+encodeURIComponent(redirectUrl),{method:"POST",body:JSON.stringify({email,create_user:true})});alert("로그인 링크를 이메일로 보냈습니다.")}catch(e){alert("로그인 요청 오류: "+e.message)}
}
...
window.addEventListener("DOMContentLoaded",setupRankingUI);
parseAuthHash();
restoreSession();
load();
