(()=>{
  ...
  function decorateCards(){
    document.querySelectorAll('.card[data-id]').forEach(card=>{
      ...
      card.querySelector('.editorialBadge')?.remove();
      if(!x)return;
  ...
  function renderEditorialDetail(){
    document.querySelector('#editorialRatingBlock')?.remove();
    const m=location.hash.match(/^#hall=(.+)$/); if(!m)return;
    const x=byHall.get(m[1]); if(!x)return;
  ...
  window.addEventListener('DOMContentLoaded',()=>setTimeout(refresh,200));
  const obs=new MutationObserver(()=>{if(loaded){decorateCards();renderEditorialDetail();renderEditorialRanking()}});
