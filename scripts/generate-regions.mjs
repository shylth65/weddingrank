...
const cards = regions.map(r => `<a class="card" href="/area/${encodeURIComponent(r.slug)}/"><b>${esc(r.name)}</b><span>${r.count.toLocaleString('ko-KR')}곳</span><em>지역별 보기 →</em></a>`).join('');
const structured = JSON.stringify({
  ...
  mainEntity:{'@type':'ItemList',numberOfItems:regions.length,itemListElement:regions.map((r,i)=>({'@type':'ListItem',position:i+1,name:r.name,url:`${SITE}/area/${encodeURIComponent(r.slug)}/`}))}
}).replace(/</g, '\\u003c');
